...
export class ExecutionService implements IService {
  public name = 'ExecutionService';
  public dependencies = ['ActionService']; 
  ...
  public async start(): Promise<void> { ExecutionWorker.getInstance().start(); this.startTime = Date.now(); this.health = HealthLevel.HEALTHY; }
  public async stop(): Promise<void> { ExecutionWorker.getInstance().stop(); this.health = HealthLevel.OFFLINE; this.metrics.uptimeMs += (Date.now() - this.startTime); }
