...
import { GestureRecognitionWorker } from '../gesture/GestureRecognitionWorker';

export class GestureService implements IService {
  public name = 'GestureService';
  public dependencies = ['TrackingService']; 
  ...
  public async start(): Promise<void> { GestureRecognitionWorker.getInstance().start(); this.startTime = Date.now(); this.health = HealthLevel.HEALTHY; }
  public async stop(): Promise<void> { GestureRecognitionWorker.getInstance().stop(); this.health = HealthLevel.OFFLINE; this.metrics.uptimeMs += (Date.now() - this.startTime); }
