...
import { IntentWorker } from '../intent/IntentWorker';

export class IntentService implements IService {
  public name = 'IntentService';
  public dependencies = ['GestureService']; 
  ...
  public async start(): Promise<void> { IntentWorker.getInstance().start(); this.startTime = Date.now(); this.health = HealthLevel.HEALTHY; }
  public async stop(): Promise<void> { IntentWorker.getInstance().stop(); this.health = HealthLevel.OFFLINE; this.metrics.uptimeMs += (Date.now() - this.startTime); }
